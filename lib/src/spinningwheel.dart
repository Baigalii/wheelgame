...
import 'package:game/src/utils.dart';
...
class _SpinningWheelState extends State<SpinningWheel>
  ...
  // divider which is selected (positive y-coordinate)
  int _currentDivider;
  ...
  void initState() {
    ...
    _animation = Tween(begin: 0.0, end: pi_2).animate(
...
//        _currentDistance = angle - 30.0;
//      });
//      _animationController.reset();
////      var angle = 360;
//////      _spinVelocity.offsetToRadians(_localPositionOnPanUpdate);
////      setState(() {
...
//  }
  ...
  _startOrStop(dynamic velocity) {
    if (_animationController.isAnimating) {
      ...
      // we assume a drag from cuadrant 1 with high velocity (8000)
      setState(() {
        _localPositionOnPanUpdate = Offset(250.0, 250.0);
        var angle = _spinVelocity.offsetToRadians(_localPositionOnPanUpdate);
        _currentDistance = angle - _initialSpinAngle;
...
//      _localPositionOnPanUpdate = Offset(250.0, 250.0);
    ...
    _updateLocalPosition(_localPositionOnPanUpdate);
  }
...
//
//    // user won't be able to get back in after drag in outside
...
//      // if more than 50 seconds passed since user dragged outside the boundaries, don't start animation
...
//  }
  ...
  void _startAnimation(Offset pixelsPerSecond) {
    ...
    _animationController.duration =
        Duration(milliseconds: (_totalDuration * 5000).round());
